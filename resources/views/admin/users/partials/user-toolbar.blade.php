<form action="{{ route('admin.users.index') }}" method="GET"
    class="grid gap-3 border-b-[1.6px] border-slate-100 p-4 md:grid-cols-2 xl:grid-cols-[minmax(280px,1fr)_160px_150px_auto]">
    ...
    <div class="relative md:col-span-2 xl:col-span-1">
        ...
        <input type="search" id="user-search" name="search" value="{{ $search }}" autocomplete="off"
            placeholder="Cari nama, email, username, nomor telepon, atau perusahaan"
    ...
    {{-- Status akun --}}
    <div>
        <label for="account-status" class="sr-only">
            Status akun
        </label>

        <select id="account-status" name="status" onchange="this.form.submit()"
    ...
    {{-- Urutan --}}
    <div>
        <label for="user-sort" class="sr-only">
            Urutan
        </label>
        ...
        <select id="user-sort" name="sort" onchange="this.form.submit()"
            ...
            <option value="newest" @selected($selectedSort === 'newest')>
                Terbaru
            </option>
